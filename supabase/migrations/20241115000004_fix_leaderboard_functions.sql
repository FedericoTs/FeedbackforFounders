-- Fix for leaderboard functions - simplified version

-- Drop existing functions if they exist to avoid conflicts
DROP FUNCTION IF EXISTS get_leaderboard;
DROP FUNCTION IF EXISTS get_user_rank_details;
...
CREATE FUNCTION get_user_rank_details(
  p_user_id UUID,
  p_timeframe TEXT DEFAULT 'all',
  p_category TEXT DEFAULT 'points'
) RETURNS SETOF JSONB AS $$
...
CREATE FUNCTION refresh_leaderboard_mv() RETURNS BOOLEAN AS $$
BEGIN
